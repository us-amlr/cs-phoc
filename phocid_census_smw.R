...
con <- dbConnect(odbc::odbc(), Driver = "SQL Server Native Client 11.0",
...
#                  Server = "swc-estrella-s", Database = "AMLR_PINNIPEDS", timeout = 10,
#                  Trusted_Connection = "Yes")

# census <- tbl(con, "vCensus") %>% collect()
...
x.out <- x.toplot %>% 
  ...
  ggtitle(i.sp)
ggsave("amlr_plots/crabby.png", x.out, width = 12, height = 8)
